              ...
              {nameEditing ? (
                <div className="flex gap-2">
                  <Input value={name} onChange={(e) => setName(e.target.value)}
                    placeholder={user!.name} className="h-8 text-sm" />
                ...
                <button type="button" onClick={() => { setName(user!.name); setNameEditing(true); }}
      ...
      {user && (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-xl bg-card border border-border px-4 py-3">
            ...
            <Select value={user.role} onValueChange={(v) => { if (v) handleRoleChange(v as "admin" | "user"); }}>
          ...
          <div className="rounded-xl bg-danger/5 border border-danger/20 p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Eliminar el usuario borra TODOS sus datos permanentemente: cuentas, tarjetas, transacciones,
              deudas, categorías, presupuestos y su cuenta de Clerk.
            </p>

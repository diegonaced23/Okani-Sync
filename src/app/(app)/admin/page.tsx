...
export default function AdminPage() {
  ...
  // Si no es admin, mostrar acceso denegado (la query retorna [] en ese caso)
  if (!isLoading && users && users.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20">
        <ShieldCheck className="h-10 w-10 text-muted-foreground" />
        <p className="text-muted-foreground">Sin acceso a esta sección.</p>
      </div>
    );
  }
          ...
          <ul className="divide-y divide-border">
            {filtered.map((user) => (
              <li key={user._id}>
                <button
                  type="button"
                  onClick={() => router.push(`/admin/usuarios/${user.clerkId}`)}

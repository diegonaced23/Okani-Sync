...
/**
 * Sin presupuestos en el mes. Los anillos que se dibujan solos adelantan el
 * lenguaje visual de la lista en vez de dejar un texto suelto en el vacío.
 */
    ...
    >
      <div className="relative mx-auto mb-6 h-32 w-32" aria-hidden="true">
        {[0.45, 0.7, 0.92].map((v, i) => (
          ...
          >
            <circle cx="50" cy="50" r="44" fill="none" strokeWidth="8" stroke="color-mix(in oklch, var(--os-lime) 18%, transparent)" />
            <motion.circle
              cx="50" cy="50" r="44" fill="none" strokeWidth="8" strokeLinecap="round"
              stroke={["var(--os-cyan)", "var(--os-orange)", "var(--os-lime)"][i]}
      ...
      <h2 className="text-lg font-extrabold tracking-tight text-foreground">
        {isCurrentMonth ? "Sin presupuestos este mes" : `Sin presupuestos en ${month}`}
      </h2>
      <p className="mx-auto mt-1.5 max-w-xs text-sm text-muted-foreground">
        Ponle un límite mensual a las categorías donde se te va el dinero y te avisamos antes de pasarte.
      </p>
      <button
        type="button"
        onClick={onCreate}
      ...
      >
        <Plus className="h-4 w-4" aria-hidden="true" /> Crear un presupuesto
      </button>
    </motion.div>
  );
}

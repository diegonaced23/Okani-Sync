...
/** Sin metas todavía. Los anillos a medio llenar insinúan a qué se parecerá esto. */
export function EmptyState({ onCreate }: { onCreate: () => void }) {
  ...
  return (
    ...
    >
        ...
        {[0.3, 0.6, 0.85].map((v, i) => (
          ...
          >
            ...
            <motion.circle
              ...
              stroke={["var(--os-violet)", "var(--os-cyan)", "var(--os-lime)"][i]}
      ...
      <h2 className="text-lg font-extrabold tracking-tight text-foreground">Aún no tienes metas</h2>
      <p className="mx-auto mt-1.5 max-w-xs text-sm text-muted-foreground">
        La laptop, el viaje o el fondo de emergencia: dale un monto y una fecha y mira cómo el anillo se llena.
      </p>
      <button
        type="button"
        onClick={onCreate}
      ...
      >
        <Plus className="h-4 w-4" aria-hidden="true" /> Crear mi primera meta
      </button>
    </motion.div>
  );
}
